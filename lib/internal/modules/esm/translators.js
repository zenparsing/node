...
function cjsSyntaxError(err) {
  return err.message === 'Illegal return statement';
}

function createMainModule(source, url) {
  ...
  try {
    return new ModuleWrap(mainModulePrelude + source, url);
  } catch (err) {
    if (cjsSyntaxError(err)) {
